...
class Gatekeeper {
  ...
  async checkPermissions(request, resolver, i18n, sellerId = null) {
    const userId = await getUserIDFromRequest(request, cache, i18n);
    ...
    if (!user || !r) {
      ...
      throw new Error('Access is denined');
    }

    // NOTE: reconstruct data
    const allRoles = user.assignment.roles ? user.assignment.roles.map(role => role.type) : [];

    let allRights = user.assignment.permissions.map(permission => permission.type);
    user.assignment.roles.forEach(role => {
      allRights = [...allRights, ...role.permissions.map(right => right.type)];
    });
    ...
    console.log(JSON.stringify(allRetailers, undefined, 1));
    // NOTE: check conditions
    if (
      sellerId &&
      (allRetailers.includes(x => x.id === sellerId) || allManufacturers.includes(y => y.id === sellerId))
    ) {
      throw new Error('Access is denied');
      console.log('passed seller');
    }

    if (_.intersection(r.requiredPermissions, allRights).length === 0) {
      throw new Error('Access is denied');
      console.log('passed rights');
    }

...
const seedTestData = async () => {
  ...
  const questions = await prisma.query.securityQuestions(
    { first: 3 },
    `{ id question }`,
  );
  ...
  seedUserTwo.confirmEmailCode = generateConfirmation(
    cache,
    seedUserTwo.user.id,
    "john2@example.com",
  );
  seedUserTwo.confirmPhoneCode = generateConfirmation(
    cache,
    seedUserTwo.user.id,
    "0123456788",
  );
...
const seedCache = async () => {
  ...
  await cache.set(
    "aaaaaa",
    JSON.stringify({ userId: "abc123", emailOrPhone: "foxiny@foxiny.com" }),
  );
  await cache.hset(
    "abc123",
